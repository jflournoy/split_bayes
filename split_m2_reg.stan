...
parameters { # Need to add cue-dimension to mu and individual priors
  ...
  corr_matrix[K] Omega_ep; // prior correlation
...
transformed parameters{
  ...
  b   = mu_b + sigma_b * b_pr; # vectorization
...
generated quantities {
  ...
  { # local section, this saves time and space
    for (i in 1:N) {
      vector[4] wv_r;  # action wegith for go
      vector[4] wv_l; # action wegith for nogo
      vector[4] qv_r;  # Q value for go
      vector[4] qv_l; # Q value for nogo
      vector[4] pR;   # prob of go (press) 
